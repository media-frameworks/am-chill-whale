...
import BurrowDiscover from "./burrow/BurrowDiscover";
import BurrowFiles from "./burrow/BurrowFiles";
import BurrowEdit from "./burrow/BurrowEdit";
...
export class FractoBurrows extends Component {
   ...
   load_registry = () => {
      BurrowFiles.load_registry(burrow_files => {
         console.log("burrow_files", burrow_files)
         this.setState({burrow_files: burrow_files})
      })
   }
   ...
   rename_burrow = (burrow_name, new_burrow_name) => {
      ...
      BurrowFiles.save_registry(burrow_files, "burrows/registry.json", result => {
         console.log("BurrowFiles.save_registry result", result)
         this.load_registry();
      })
   }

   render() {
      const {discover_mode, burrow_files, selected_burrow, in_edit_name} = this.state;
      const {width_px} = this.props;

      const title_bar = render_title_bar("bureau of Burrows");
      const main_link = render_main_link("new burrow", e => this.setState({discover_mode: true}));

      const discover_modal = !discover_mode ? '' : <BurrowDiscover
         burrow_files={burrow_files}
         on_response_modal={r => this.response_modal(r)}/>;
      const burrows = Object.keys(burrow_files)
         .sort()
         .map(burrow_name => {
